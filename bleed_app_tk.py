...
class FlexCutWindow(customtkinter.CTkToplevel):
    ...
    def __init__(self, parent, job, sheet_pdfs, bleed_mm, on_reexport):
        ...
        customtkinter.CTkButton(
            toolbar, text="Dodaj FlexCut (Z)", width=130, height=28, corner_radius=6,
        ...
        customtkinter.CTkButton(
            toolbar, text="Obróć 180° (R)", width=110, height=28, corner_radius=6,
            fg_color="#607d8b", hover_color="#455a64",
            text_color="white", font=customtkinter.CTkFont(size=11, weight="bold"),
            command=self._on_rotate_180,
        ).pack(side="right", padx=4)

        customtkinter.CTkButton(
            toolbar, text="Dodaj spad (S)", width=110, height=28, corner_radius=6,
            fg_color="#8e24aa", hover_color="#6a1b9a",
            text_color="white", font=customtkinter.CTkFont(size=11, weight="bold"),
            command=self._on_add_bleed_rect,
        ).pack(side="right", padx=4)
        ...
        self.canvas.bind("<Double-Button-1>", self._on_zoom_reset)
        # Skróty klawiszowe
        self.bind("<z>", lambda e: self._on_apply())    # Z = Dodaj FlexCut
        self.bind("<Z>", lambda e: self._on_apply())
        self.bind("<r>", lambda e: self._on_rotate_180())  # R = Obróć 180°
        self.bind("<R>", lambda e: self._on_rotate_180())
        self.bind("<s>", lambda e: self._on_add_bleed_rect())  # S = Dodaj spad
        self.bind("<S>", lambda e: self._on_add_bleed_rect())
    ...
    def _draw_flexcut_selection(self, sheet, tx, ty):
        ...
        for idx, p in enumerate(sheet.placements):
            ...
            # Wizualny marker obrotu 180°/270° (↻)
            rot = int(p.rotation_deg) % 360
            if rot in (180, 270):
                cx = (tx(p.x_mm) + tx(p.x_mm + pw)) / 2
                cy = (ty(p.y_mm + ph) + ty(p.y_mm)) / 2
                self.canvas.create_text(
                    cx, cy, text="↻", font=("Arial", 14, "bold"),
                    fill="#ff9800", anchor="center",
                )
        ...
        # Rysuj linie z panel_lines
        for line in sheet.panel_lines:
            is_fullcut = line.bridge_length_mm <= 0
            color = _PREVIEW_CUTCONTOUR if is_fullcut else _PREVIEW_FLEXCUT
            dash = () if is_fullcut else (5, 3)
            lw = 2.5 if is_fullcut else 2
            if line.axis == "horizontal":
                y = ty(line.position_mm)
                x0 = tx(line.start_mm)
                x1 = tx(line.end_mm)
                self.canvas.create_line(x0, y, x1, y, fill=color, width=lw, dash=dash)
            elif line.axis == "vertical":
                x = tx(line.position_mm)
                y0 = ty(line.start_mm)
                y1 = ty(line.end_mm)
                self.canvas.create_line(x, y0, x, y1, fill=color, width=lw, dash=dash)
    ...
    def _on_rotate_180(self):
        """Obraca zaznaczone naklejki o 180° — re-export + odświeżenie podglądu."""
        if not self._selected_placements:
            self._app_log("Obróć 180°: brak zaznaczonych naklejek")
            return
        if not self.job or not self.job.sheets:
            return
        sheet = self.job.sheets[self.current_sheet_idx]
        count = 0
        for idx in self._selected_placements:
            if idx < len(sheet.placements):
                p = sheet.placements[idx]
                p.rotation_deg = (p.rotation_deg + 180) % 360
                count += 1
        self._app_log(f"Obróć 180°: {count} naklejek")
        self._selected_placements.clear()
        # Re-export + odświeżenie podglądu
        if self._on_reexport:
            self._render_cache.clear()
            self._on_reexport(self.current_sheet_idx)

    def _on_add_bleed_rect(self):
        """Dodaj spad — 2mm rozszerzenie grafiki + linia cięcia wokół naklejek.

        Spad = fizyczne rozszerzenie grafiki o 2mm kolorem z krawędzi sąsiadujących
        naklejek (nearest-neighbor dilation). Uwzględnia aktualny stan obrotów.
        Linia cięcia na granicy naklejek (CutContour, bridge=0).
        """
        from models import PanelLine
        if not self.job or not self.job.sheets:
            self._app_log("Spad: brak arkuszy")
            return
        sheet = self.job.sheets[self.current_sheet_idx]
        if not sheet.placements:
            self._app_log("Spad: brak naklejek")
            return
        ...
        # Bbox WSZYSTKICH naklejek na arkuszu (footprint z bleedem)
        x0 = min(p.x_mm for p in sheet.placements)
        y0 = min(p.y_mm for p in sheet.placements)
        x1 = max(p.x_mm + _pw(p) + bleed2 for p in sheet.placements)
        y1 = max(p.y_mm + _ph(p) + bleed2 for p in sheet.placements)

        # Ustaw outer_bleed na arkuszu — export_sheet_print wygeneruje dilation
        sheet.outer_bleed_mm = 2.0

        # Linia cięcia na granicy naklejek (CutContour, pełne cięcie)
        # Usun stare linie spadu (bridge=0) przed dodaniem nowych
        ...
        self._app_log(f"Spad: 2mm dilation + linia cięcia ({x0:.1f},{y0:.1f})-({x1:.1f},{y1:.1f})mm")

        # Re-export + odświeżenie
        if self._on_reexport:
            self._render_cache.clear()
            self._on_reexport(self.current_sheet_idx)
    ...
    def _on_clear(self):
        """Wyczyść FlexCut, spad i selekcję na bieżącym arkuszu."""
        if not self.job or not self.job.sheets:
            return
        idx = self.current_sheet_idx
        sheet = self.job.sheets[idx]
        ...
        self._selected_placements.clear()
        self._app_log("Wyczyszczono: FlexCut + spad")
        # Re-export żeby usunąć spad z PDF
        if self._on_reexport:
            self._render_cache.clear()
            self._on_reexport(self.current_sheet_idx)
        else:
    ...
    def _on_export_flexcut(self):
        ...
        if exported:
            self._app_log(f"Zastosuj: wyeksportowano {exported} arkusz(y)")
        else:
            self._app_log("Zastosuj: brak arkuszy do eksportu")
...
class BleedApp(customtkinter.CTk):
    ...
    def _build_ui(self):
        ...
        # Lewy panel: content (ustawienia, pliki + log)
        ...
        # Log — placeholder, zostanie umieszczony wewnątrz _content per zakładka
        # (pod przyciskami "Generuj bleed" / "Generuj arkusze")
        self._log_text = None  # tworzony w _build_log_widget()

        # Prawy panel: preview (pełna wysokość)
    ...
    def _show_bleed_tab(self):
        ...
        # Log — wypełnia resztę lewego panelu pod ustawieniami
        self._build_log_widget(self._content_outer)
    ...
    def _show_nest_tab(self):
        ...
        # -- Roll frame (szerokość rolki + max długość + dodaj/usuń)
        self._nest_roll_frame = customtkinter.CTkFrame(self._nest_format_container, fg_color="transparent")
        self._roll_widths = list(ROLL_PRESETS)  # edytowalna lista
        roll_values = [str(w) for w in self._roll_widths]
        self._roll_width_var = customtkinter.StringVar(value=roll_values[0] if roll_values else "1370")
        self._roll_combo = customtkinter.CTkComboBox(
            self._nest_roll_frame, variable=self._roll_width_var,
            values=roll_values, width=85,
        )
        self._roll_combo.pack(side="left", padx=(0, 2))
        customtkinter.CTkButton(
            self._nest_roll_frame, text="+", width=24, height=24,
            font=customtkinter.CTkFont(size=13, weight="bold"),
            fg_color=ACCENT, hover_color=ACCENT_HOVER, text_color="white",
            command=self._roll_add_width,
        ).pack(side="left", padx=(0, 1))
        customtkinter.CTkButton(
            self._nest_roll_frame, text="−", width=24, height=24,
            font=customtkinter.CTkFont(size=13, weight="bold"),
            fg_color="#e53935", hover_color="#c62828", text_color="white",
            command=self._roll_remove_width,
        ).pack(side="left", padx=(0, 6))
        ...
        customtkinter.CTkButton(
            r, text="Max", width=40, height=24, corner_radius=4,
            font=customtkinter.CTkFont(size=10, weight="bold"),
            fg_color="#607d8b", hover_color="#455a64", text_color="white",
            command=self._calc_max_copies,
    ...
    def _calc_max_copies(self):
        """Oblicza max kopii na arkuszu i ustawia pole Kopie.

        Sprawdza oba ułożenia (normalne + obrót 90°) i wybiera lepsze.
        Uwzględnia bleed, gap, margines i mark_zone.
        """
        import fitz as fitz_module
        from config import PLOTTERS, DEFAULT_BLEED_MM, DEFAULT_MARK_ZONE_MM
        ...
        # Rozmiar arkusza
        mode = self._nest_mode_var.get() if hasattr(self, '_nest_mode_var') else "Arkusze"
        try:
            if mode == "Arkusze":
                preset = self._sheet_var.get()
                from config import SHEET_PRESETS
                if preset in SHEET_PRESETS:
                    sheet_w, sheet_h = SHEET_PRESETS[preset]
                else:
                    sheet_w, sheet_h = 320, 450
            else:
                sheet_w = float(self._roll_width_var.get())
                sheet_h = float(self._roll_max_len_var.get())
        except (ValueError, AttributeError):
            self._log("Max: nie mogę odczytać rozmiaru arkusza")
            return
        ...
        plotter = self._plotter_var.get() if hasattr(self, '_plotter_var') else "jwei"
        ...
        # Odczytaj rozmiary footprintów z WSZYSTKICH plików
        # Nesting dodaje bleed do footprintu: sticker_mm + 2*bleed
        # Dla plików bleed_: sticker_mm = page_mm - 2*bleed (odejmuje), potem dodaje → page_mm
        # Dla zwykłych plików: sticker_mm = page_mm, footprint = page_mm + 2*bleed
        # W obu przypadkach footprint w nestingu = page_mm + 2*bleed dla zwykłych,
        # page_mm dla bleed_ (odejmij i dodaj = to samo)
        # Więc: footprint = rozmiar pliku (bo nesting odejmuje i dodaje bleed)
        # Nest: footprint = rozmiar strony (surowe pliki bez bleeda, bleed_ as-is)
        footprints = []
        for filepath in self._files:
            try:
                doc = fitz_module.open(filepath)
                page = doc[0]
                fw = page.rect.width * PT_TO_MM
                fh = page.rect.height * PT_TO_MM
                doc.close()
                file_copies = self._file_copies.get(filepath, 1)
                footprints.append((fw, fh, file_copies))
            except Exception:
                continue

        if not footprints:
            self._log("Max: brak plików do obliczenia")
            return

        # Oblicz max kopii — prostokątny grid
        # Dla jednego wzoru: ile zmieści się na arkuszu
        # Dla wielu wzorów: oblicz ile jednego wzoru (per plik)
        import math
        def count_fit(w, h, aw, ah, g):
            if w <= 0 or h <= 0:
                return 0
            # Tolerancja 0.01mm — kompensacja błędów float (np. 100.00001mm)
            nx = int(math.floor((aw + g) / (w + g) + 0.01))
            ny = int(math.floor((ah + g) / (h + g) + 0.01))
            return nx * ny

        if len(set(self._files)) == 1 or len(footprints) == 1:
            # Jeden wzór — oblicz max kopii z obrotem
            fw, fh = footprints[0][0], footprints[0][1]
            ...
            self._copies_var.set(str(n_max))
            self._log(f"Max: {n_max} kopii ({orient}, {fw:.0f}×{fh:.0f}mm na {avail_w:.0f}×{avail_h:.0f}mm)")
        else:
            # Wiele wzorów — oblicz per największy footprint
            max_fw = max(fp[0] for fp in footprints)
            max_fh = max(fp[1] for fp in footprints)
            n_normal = count_fit(max_fw, max_fh, avail_w, avail_h, gap)
            n_rotated = count_fit(max_fh, max_fw, avail_w, avail_h, gap)
            n_max = max(n_normal, n_rotated, 1)
            self._copies_var.set(str(n_max))
            self._log(f"Max: {n_max} kopii (wg największego wzoru {max_fw:.0f}×{max_fh:.0f}mm)")

    def _roll_add_width(self):
        """Dodaje aktualną wartość z ComboBox do listy szerokości rolek."""
        try:
            val = int(float(self._roll_width_var.get()))
        except ValueError:
            return
        if val < 50 or val > 5000:
            return
        if val not in self._roll_widths:
            self._roll_widths.append(val)
            self._roll_widths.sort()
        self._roll_combo.configure(values=[str(w) for w in self._roll_widths])
        self._roll_width_var.set(str(val))
        self._log(f"Rolka: dodano szerokość {val}mm")

    def _roll_remove_width(self):
        """Usuwa aktualną wartość z listy szerokości rolek."""
        try:
            val = int(float(self._roll_width_var.get()))
        except ValueError:
            return
        if val in self._roll_widths and len(self._roll_widths) > 1:
            ...
            self._roll_combo.configure(values=[str(w) for w in self._roll_widths])
            self._roll_width_var.set(str(self._roll_widths[0]))
    ...
    def _build_bleed_settings(self, parent):
        ...
        # Linia cięcia — radio: Kiss-Cut / FlexCut / Brak
        cut_frame = customtkinter.CTkFrame(body, fg_color="transparent")
        cut_frame.pack(anchor="w", pady=2)
        customtkinter.CTkLabel(
            cut_frame, text="Linia cięcia:",
            font=customtkinter.CTkFont(size=12), text_color=TEXT_SECONDARY,
        ).pack(side="left", padx=(0, 8))
        self._cutline_var = customtkinter.StringVar(value="kiss-cut")
        for val, label in [("kiss-cut", "Kiss-Cut"), ("flexcut", "FlexCut"), ("none", "Brak")]:
            customtkinter.CTkRadioButton(
                cut_frame, text=label, variable=self._cutline_var, value=val,
                font=customtkinter.CTkFont(size=11),
    ...
    def _on_run(self):
        ...
        cutline_mode = self._cutline_var.get()  # "kiss-cut", "flexcut", "none"
        ...
        thread = threading.Thread(
            ...
            args=(list(self._files), self._output_dir, bleed_mm, black_100k,
                  cutcontour, cutline_mode, target_height_mm, crop_enabled, crop_shape,
                ...
                for sticker in stickers:
                    ...
                    try:
                        ...
                        info = export_single_sticker(
                            ...
                            cutline_mode=cutline_mode, white=white,
                        )
    ...
    def _worker_nest(self, params: dict):
        ...
        for i, pdf in enumerate(input_files):
            ...
            # Nest: pliki traktowane jako gotowe — rozmiar strony = footprint
            # Bez detect_contour, bez generate_bleed
            ...
            try:
                if True:
                    ...
                    # Nest nie dodaje bleeda — plik = gotowy footprint
                    b = 0
                    b_pt = b * mm_to_pt
                    ...
                    # Brak CutContour w PDF → brak linii cięcia (Nest nie generuje własnych)
                    if cut_segs is None:
                        cut_segs = []
                        self._log(f"    CutContour: brak w PDF — bez linii cięcia")
                    ...
                    self._log(f"  {name} ({tag}): {pw_mm:.1f}x{ph_mm:.1f}mm x{copies}")
            except Exception as e:
                self._log(f"  [ERR] {name}: {e}")
        ...
        # 2. Nesting (z ochroną wyjątków — błąd nie zabije wątku)
        ...
        try:
            self._worker_nest_inner(params, sticker_copies_list, open_docs,
                                    grouping_mode, plotter, bleed, gap, white, out_dir, t0)
        except Exception as e:
            import traceback
            tb = traceback.format_exc()
            self._log(f"\n[BŁĄD KRYTYCZNY] Nesting/eksport nie powiódł się:\n  {e}\n{tb}")
            for doc in open_docs:
                try: doc.close()
                except Exception: pass
            self.after(0, lambda: self._on_nest_done([]))

    def _worker_nest_inner(self, params, sticker_copies_list, open_docs,
                           grouping_mode, plotter, bleed, gap, white, out_dir, t0):
        """Właściwa logika nestingu — wydzielona żeby try/except łapał wszystko."""
        from modules.nesting import nest_job
        from modules.panelize import panelize_sheet
        from modules.marks import generate_marks
        from modules.export import export_sheet
        ...
        job = nest_job(
            ...
            bleed_mm=0,  # pliki w Nest = gotowe footprinty, bez dodawania bleeda
        ...
        for i, sheet in enumerate(job.sheets):
            ...
            export_sheet(sheet, pp, cp, bleed_mm=0, plotter=plotter,
        ...
        self._last_nest_bleed = 0  # Nest: pliki = gotowe footprinty
    ...
    def _reexport_sheet(self, idx: int):
        ...
        try:
            ...
            export_sheet(sheet, pp, cp, bleed_mm=0, plotter=job.plotter,
                         white=white, white_output_path=wp)
            ...
            # Odśwież podgląd FlexCut (jeśli otwarty)
            fw = getattr(self, '_flexcut_window', None)
            if fw and fw.winfo_exists():
                fw._render_cache.clear()
                fw._draw_current_sheet()
        except Exception as e:
    ...
    def _build_log_widget(self, parent):
        """Tworzy (lub przenosi) widget logu do podanego parent frame."""
        if self._log_text is not None:
            try:
                ...
                pass
            try:
                self._log_text.destroy()
            except Exception:
                pass
        self._log_text = customtkinter.CTkTextbox(
            parent, height=160, fg_color=LOG_BG, text_color=LOG_FG,
            font=customtkinter.CTkFont(family="Consolas", size=11),
            state="disabled",
        )
        self._log_text.pack(fill="both", expand=True, padx=16, pady=(6, 10))
    ...
    def _flush_log(self):
        ...
        try:
            self._log_text._textbox.tag_configure("error", foreground="#e53935")
        except Exception:
            pass
        for msg in messages:
            is_error = any(kw in msg for kw in ("[ERR]", "[BŁĄD", "ERROR", "Traceback", "BLAD"))
            if is_error:
                try:
